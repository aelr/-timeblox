...
export const createTimBox = (startDay, startHour, startMinute, offsetHours, offsetMinutes) => {
    return { startDay, startHour, startMinute, offsetHours, offsetMinutes };
};
/**
 ...
 * @returns {number}
 */
export const encodeTimeBox = (timeBox) => {
 ...
 * @returns TimeBlock
 */
export const decodeTimeBox = (encoded) => {
    const start = (((encoded & 0x1FFE00) >>> 9) * 5) % MINUTES_IN_WEEK;
    const startDay = Math.floor(start / MINUTES_IN_DAY);
